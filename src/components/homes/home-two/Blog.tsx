      ...
      <div className='container'>
        <div className='position-relative'>
                ...
                <div key={item.id} className='col-md-6'>
                  <article className='blog-meta-one mt-35 wow fadeInUp' data-wow-delay={item.data_delay_time}>
                    <figure className={`post-img position-relative m0 ${item.class_name} ${style ? 'rounded-5' : ''}`}>
                      <Link href='/blog_details' className={`stretched-link date tran3s ${style ? 'rounded-5' : ''}`}>
                        {item.date}
                      </Link>
                    </figure>
                    <div className='post-data'>
                      <div className='post-info'>
                        <Link href='/blog_details'>{item.info_name}</Link> {item.info_time} min
                      </div>
          ...
          <div className='section-btn text-center md-mt-60'>
            <Link href='/blog_03' className='btn-eight'>
