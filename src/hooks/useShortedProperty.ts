...
const UseShortedProperty = ({itemsPerPage, page}: DataType) => {
  ...
  // handleSortOptionChange adaptado para NiceSelect
  const handleTypeChange = (option: {value: string; text: string}) => {
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    const searchingProducts = filteredAllProduct.filter((p) => p.title.toLowerCase().includes(e.target.value.toLowerCase()));
